...
try: # try to load images from the harddisk
    prettybackground = pygame.image.load(os.path.join(folder, "../img/png/player.png"))
    uglybackground = pygame.image.load(os.path.join(folder, "../img/png/playerSprite.png"))
    snakesurface = pygame.image.load(os.path.join(folder,"../img/png/player.png")) # with tranparent colour
except:
...
while mainloop:
    ...
    try:
        screen = prettybackground.subsurface((x,y,snakerect.width, snakerect.height))
    except:
        print("some problem with subsurface")
    screen.blit(screen, (0,0)) # blit into screen like a tv 
    if painting:
        background.blit(screen, (x,y)) # blit from pretty background into background
